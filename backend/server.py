...
async def _get_org(org_id: str) -> dict:
    ...
    return org or {"org_id": org_id, "plan": "free", "plan_status": "active"}

async def _check_plan_limit(org_id: str, resource: str):
    """Raises HTTP 403 if the org has reached its plan limit for the given resource."""
    org = await _get_org(org_id)
    plan = org.get("plan", "free")
    limit = PLAN_LIMITS.get(plan, PLAN_LIMITS["free"]).get(resource, 0)
    if limit == -1:
        return  # unlimited
    count = await db[resource if resource != "leads" else "leads"].count_documents({"org_id": org_id}) if resource in ("leads",) else None
    if resource == "clients":
        count = await db.clients.count_documents({"org_id": org_id})
    elif resource == "leads":
        count = await db.leads.count_documents({"org_id": org_id})
    ...
    if count is not None and count >= limit:
        ...
        raise HTTPException(
            status_code=403,
            detail=f"Limite do plano {plan_label} atingido: máximo de {limit} {resource}. Faça upgrade para continuar."
        )

# ============= AUTH UTILITIES =============

def create_jwt_token(user_id: str, email: str, org_id: str = "", role: str = "admin") -> str:
    payload = {
        "user_id": user_id,
        "org_id": org_id,
        "role": role,
...
class CarouselThemesRequest(BaseModel):
    client_id: str
    news_context: str = Field(..., max_length=20000)
    llm_provider: str = "openai"

class CarouselCopyRequest(BaseModel):
    client_id: str
    chosen_theme: str = Field(..., max_length=500)
    news_context: str = Field(..., max_length=20000)
    llm_provider: str = "openai"

class CarouselDesignRequest(BaseModel):
    client_id: str
    copy_content: str = Field(..., max_length=20000)
    chosen_theme: str = Field(..., max_length=500)
    llm_provider: str = "openai"
    template_image_b64: Optional[str] = Field(None, max_length=6_700_000)  # ~5MB base64
    template_image_type: Optional[str] = None  # "image/jpeg", "image/png", "image/webp"
    change_request: Optional[str] = Field(None, max_length=20000)
    current_html: Optional[str] = Field(None, max_length=200000)
...
async def register(request: Request, body: RegisterRequest, response: Response):
    ...
    token = create_jwt_token(user_id, body.email, org_id, "admin")
...
async def login(request: Request, body: LoginRequest, response: Response):
    ...
    token = create_jwt_token(user["user_id"], user["email"], org_id, role)
    response.set_cookie("token", token, httponly=True, samesite="none", secure=True, max_age=7 * 24 * 3600, path="/")
...
async def google_auth_session(body: SessionRequest, response: Response):
    ...
    if existing:
        ...
        org_id = existing.get("org_id", existing["user_id"])
    ...
    token = create_jwt_token(user_id, email, org_id, role)
    response.set_cookie("token", token, httponly=True, samesite="none", secure=True, max_age=7 * 24 * 3600, path="/")
    return {"token": token, "user": user_data}
...
@api_router.post("/org/members/invite")
async def invite_member(body: OrgInviteRequest, current_user: dict = Depends(get_current_user)):
    if current_user.get("role") != "admin":
        raise HTTPException(status_code=403, detail="Apenas administradores podem convidar membros")
    await _check_plan_limit(current_user["org_id"], "members")
    existing = await db.users.find_one({"email": body.email}, {"_id": 0})
    if existing:
        raise HTTPException(status_code=400, detail="Email já cadastrado")
    now = datetime.now(timezone.utc).isoformat()
    # Generate a temporary invite token (24h) that the user uses to set their password
    ...
    await db.users.insert_one(user_doc)
    # In production: send email with invite link containing invite_token
    return {
        "user_id": user_id,
        "email": body.email,
        "invite_token": invite_token,
        "message": "Convite criado. Envie o link de aceite ao membro."
    }

@api_router.post("/auth/accept-invite")
async def accept_invite(token: str, password: str, response: Response):
    ...
    await db.users.update_one(
        {"invite_token": token},
        {"$set": {"password_hash": password_hash, "active": True, "invite_token": None, "invite_expires_at": None}}
    )
    org_id = user["org_id"]
    role = user.get("role", "member")
    jwt_token = create_jwt_token(user["user_id"], user["email"], org_id, role)
    response.set_cookie("token", jwt_token, httponly=True, samesite="none", secure=True, max_age=7 * 24 * 3600, path="/")
    user_data = {k: v for k, v in user.items() if k not in ["_id", "password_hash", "invite_token", "invite_expires_at"]}
    return {"token": jwt_token, "user": user_data}

@api_router.delete("/org/members/{user_id}")
async def remove_member(user_id: str, current_user: dict = Depends(get_current_user)):
    ...
    result = await db.users.delete_one({"user_id": user_id, "org_id": current_user["org_id"]})
...
async def list_leads(current_user: dict = Depends(get_current_user)):
    leads = await db.leads.find({"org_id": current_user["org_id"]}, {"_id": 0}).sort("created_at", -1).to_list(500)
    return leads


@api_router.post("/leads")
async def create_lead(body: LeadCreate, current_user: dict = Depends(get_current_user)):
    await _check_plan_limit(current_user["org_id"], "leads")
...
async def get_lead(lead_id: str, current_user: dict = Depends(get_current_user)):
    lead = await db.leads.find_one({"lead_id": lead_id, "org_id": current_user["org_id"]}, {"_id": 0})
    if not lead:
...
async def update_lead(lead_id: str, body: LeadUpdate, current_user: dict = Depends(get_current_user)):
    ...
    result = await db.leads.update_one({"lead_id": lead_id, "org_id": current_user["org_id"]}, {"$set": update_data})
    if result.matched_count == 0:
        raise HTTPException(status_code=404, detail="Lead não encontrado")
    return await db.leads.find_one({"lead_id": lead_id, "org_id": current_user["org_id"]}, {"_id": 0})
...
class AsaasSettings(BaseModel):
    ...
    sandbox: bool = True
    enabled: bool = False
    notify_before_due_email: bool = False
    notify_before_due_sms: bool = False
    notify_before_due_whatsapp: bool = False
    notify_overdue_email: bool = False
    notify_overdue_sms: bool = False
    notify_overdue_whatsapp: bool = False
    notify_paid_email: bool = False
    notify_paid_sms: bool = False
    notify_paid_whatsapp: bool = False
        ...
        async with httpx.AsyncClient(timeout=20.0) as http:
            ...
            if client_doc.get("monthly_value", 0) > 0 and result["asaas_customer_id"]:
                ...
                if resp2.status_code in (200, 201):
                    ...
                    # Configure customer notifications per saved settings
                    NOTIF_GROUP = {
                        "PAYMENT_CREATED": "before_due",
                        "PAYMENT_UPDATED": "before_due",
                        "PAYMENT_REMINDER": "before_due",
                        "PAYMENT_DUE": "before_due",
                        "PAYMENT_OVERDUE": "overdue",
                        "PAYMENT_OVERDUE_REMINDER": "overdue",
                        ...
                        if notif_resp.status_code == 200:
                            for notif in notif_resp.json().get("data", []):
                                ...
                                await http.put(
                                    ...
                                    json={
                                        "enabled": True,
                                        "emailEnabledForCustomer": settings.get(f"notify_{group}_email", False),
                                        "smsEnabledForCustomer": settings.get(f"notify_{group}_sms", False),
                                        "whatsappEnabledForCustomer": settings.get(f"notify_{group}_whatsapp", False),
                                    },
                                    headers=headers,
                                )
                else:
...
async def get_asaas_settings(current_user: dict = Depends(get_current_user)):
    ...
    return {
        ...
        "notify_before_due_email": s.get("notify_before_due_email", False),
        "notify_before_due_sms": s.get("notify_before_due_sms", False),
        "notify_before_due_whatsapp": s.get("notify_before_due_whatsapp", False),
        "notify_overdue_email": s.get("notify_overdue_email", False),
        "notify_overdue_sms": s.get("notify_overdue_sms", False),
        "notify_overdue_whatsapp": s.get("notify_overdue_whatsapp", False),
        "notify_paid_email": s.get("notify_paid_email", False),
        "notify_paid_sms": s.get("notify_paid_sms", False),
        "notify_paid_whatsapp": s.get("notify_paid_whatsapp", False),
    }
...
async def save_asaas_settings(body: AsaasSettings, current_user: dict = Depends(get_current_user)):
    update: dict = {
        ...
        "notify_before_due_email": body.notify_before_due_email,
        "notify_before_due_sms": body.notify_before_due_sms,
        "notify_before_due_whatsapp": body.notify_before_due_whatsapp,
        "notify_overdue_email": body.notify_overdue_email,
        "notify_overdue_sms": body.notify_overdue_sms,
        "notify_overdue_whatsapp": body.notify_overdue_whatsapp,
        "notify_paid_email": body.notify_paid_email,
        "notify_paid_sms": body.notify_paid_sms,
        "notify_paid_whatsapp": body.notify_paid_whatsapp,
...
async def list_clients(current_user: dict = Depends(get_current_user)):
    clients = await db.clients.find({"org_id": current_user["org_id"]}, {"_id": 0}).sort("created_at", -1).to_list(500)
    return clients


@api_router.post("/clients")
async def create_client(body: ClientCreate, current_user: dict = Depends(get_current_user)):
    await _check_plan_limit(current_user["org_id"], "clients")
    client_id = f"client_{uuid.uuid4().hex[:10]}"
    now = datetime.now(timezone.utc).isoformat()
    client_doc = {
        "client_id": client_id,
...
async def update_client(client_id: str, body: ClientUpdate, current_user: dict = Depends(get_current_user)):
    update_data = {k: v for k, v in body.model_dump().items() if v is not None}
    update_data["updated_at"] = datetime.now(timezone.utc).isoformat()
    result = await db.clients.update_one({"client_id": client_id, "org_id": current_user["org_id"]}, {"$set": update_data})
    if result.matched_count == 0:
        raise HTTPException(status_code=404, detail="Cliente não encontrado")
    return await db.clients.find_one({"client_id": client_id, "org_id": current_user["org_id"]}, {"_id": 0})
...
async def generate_carousel(body: CarouselRequest, current_user: dict = Depends(get_current_user)):
    client = await db.clients.find_one({"client_id": body.client_id, "org_id": current_user["org_id"]}, {"_id": 0})
    if not client:
        raise HTTPException(status_code=404, detail="Cliente não encontrado")
...
async def update_task(task_id: str, body: TaskUpdate, current_user: dict = Depends(get_current_user)):
    ...
    task = await db.operational_tasks.find_one({"task_id": task_id, "org_id": current_user["org_id"]}, {"_id": 0})
    if task:
        task["assignee"] = (
...
async def delete_task(task_id: str, current_user: dict = Depends(get_current_user)):
    now = datetime.now(timezone.utc).isoformat()
    result = await db.operational_tasks.update_one(
        {"task_id": task_id, "org_id": current_user["org_id"]}, {"$set": {"deleted_at": now, "updated_at": now}}
    )
    if result.matched_count == 0:
        raise HTTPException(status_code=404, detail="Tarefa não encontrada")
...
async def get_operational_summary(manager_id: Optional[str] = None, current_user: dict = Depends(get_current_user)):
    clients = await db.clients.find({"org_id": current_user["org_id"], "status": "ativo"}, {"_id": 0}).to_list(500)
    if not clients:
        return []
...
async def get_api_keys(current_user: dict = Depends(get_current_user)):
    doc = await db.user_api_keys.find_one({"org_id": current_user["org_id"]}, {"_id": 0})
    if not doc:
...
async def update_api_keys(body: ApiKeysUpdate, current_user: dict = Depends(get_current_user)):
    ...
    if update_data:
        await db.user_api_keys.update_one(
            {"org_id": current_user["org_id"]},
            {"$set": {**update_data, "org_id": current_user["org_id"]}},
            upsert=True
        )
    return {"status": "ok"}
...
async def _get_api_key(org_id: str, provider: str) -> str:
    doc = await db.user_api_keys.find_one({"org_id": org_id}, {"_id": 0})
    ...
    async with httpx.AsyncClient(timeout=180) as client:
        if provider == "openai":
            r = await client.post(
                ...
                headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"},
                json={"model": "gpt-4o", "messages": [{"role": "system", "content": system_prompt}, {"role": "user", "content": user_prompt}], "temperature": 0.7, "max_tokens": 16000},
            )
        elif provider == "anthropic":
            r = await client.post(
                "https://api.anthropic.com/v1/messages",
                headers={"x-api-key": api_key, "anthropic-version": "2023-06-01", "Content-Type": "application/json"},
                json={"model": "claude-sonnet-4-6", "max_tokens": 16000, "system": system_prompt, "messages": [{"role": "user", "content": user_prompt}]},
            )
        elif provider == "gemini":
            r = await client.post(
                f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key={api_key}",
                headers={"Content-Type": "application/json"},
                json={"contents": [{"parts": [{"text": f"{system_prompt}\n\n{user_prompt}"}]}], "generationConfig": {"temperature": 0.7, "maxOutputTokens": 8192}},
            )
        elif provider == "groq":
            r = await client.post(
                "https://api.groq.com/openai/v1/chat/completions",
                headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"},
                json={"model": "llama-3.3-70b-versatile", "messages": [{"role": "system", "content": system_prompt}, {"role": "user", "content": user_prompt}], "temperature": 0.7, "max_tokens": 8000},
            )
        else:
...
async def carousel_agent_news(request: Request, body: CarouselNewsRequest, current_user: dict = Depends(get_current_user)):
    api_key = await _get_api_key(current_user["org_id"], body.llm_provider)
    client = await db.clients.find_one({"client_id": body.client_id, "org_id": current_user["org_id"]}, {"_id": 0})
    if not client:
        raise HTTPException(status_code=404, detail="Cliente não encontrado")
    ...
    await db.carousel_agent_jobs.insert_one({
        "job_id": job_id, "org_id": current_user["org_id"], "step": "news",
...
@limiter.limit("10/minute")
async def carousel_agent_themes(request: Request, body: CarouselThemesRequest, current_user: dict = Depends(get_current_user)):
    api_key = await _get_api_key(current_user["org_id"], body.llm_provider)
    client = await db.clients.find_one({"client_id": body.client_id, "org_id": current_user["org_id"]}, {"_id": 0})
    ...
    job_id = f"agentjob_{uuid.uuid4().hex[:12]}"
    await db.carousel_agent_jobs.insert_one({
        "job_id": job_id, "org_id": current_user["org_id"], "step": "themes",
        "status": "pending", "result": None, "error": None,
        "created_at": datetime.now(timezone.utc).isoformat(),
    })
...
@limiter.limit("10/minute")
async def carousel_agent_copy(request: Request, body: CarouselCopyRequest, current_user: dict = Depends(get_current_user)):
    api_key = await _get_api_key(current_user["org_id"], body.llm_provider)
    client = await db.clients.find_one({"client_id": body.client_id, "org_id": current_user["org_id"]}, {"_id": 0})
    niche = ((client.get("company") or client.get("name", "negócio")) if client else "negócio")
    ...
    job_id = f"agentjob_{uuid.uuid4().hex[:12]}"
    await db.carousel_agent_jobs.insert_one({
        "job_id": job_id, "org_id": current_user["org_id"], "step": "copy",
        "status": "pending", "result": None, "error": None,
        "created_at": datetime.now(timezone.utc).isoformat(),
    })
...
async def _run_carousel_design_job(job_id: str, body: CarouselDesignRequest, api_key: str, client_name: str, client_notes: str):
    """Background task to generate carousel HTML design using LLM."""
    try:
        has_template = bool(body.template_image_b64)

        # ──────────────────────────────────────────────────────────────────
        # MANDATORY JS TEMPLATE — included verbatim in every prompt
        # CSS is injected into <head> FIRST so LLM styles override it.
  ...
  if (slides.length > 0) goTo(0);
        ...
        # Combined for use in prompts (CSS first so LLM styles take precedence)
        JS_TEMPLATE = JS_CSS + "\n" + JS_SCRIPT
        ...
        # ALWAYS ensure navigation script is present.
        # CSS defaults go into <head> FIRST (so LLM styles override them).
        # Script goes before </body>.
        if html and 'goTo' not in html:
            # Inject base CSS into <head> before LLM styles so LLM CSS wins
            if '<head>' in html:
                html = html.replace('<head>', f'<head>\n{JS_CSS}', 1)
            # Inject script before </body>
            if '</body>' in html.lower():
                idx = html.lower().rfind('</body>')
                html = html[:idx] + JS_SCRIPT + '\n' + html[idx:]
            else:
...
@limiter.limit("10/minute")
async def carousel_agent_design(request: Request, body: CarouselDesignRequest, current_user: dict = Depends(get_current_user)):
    api_key = await _get_api_key(current_user["org_id"], body.llm_provider)
    client = await db.clients.find_one({"client_id": body.client_id, "org_id": current_user["org_id"]}, {"_id": 0})
...
async def get_carousel_detail(carousel_id: str, current_user: dict = Depends(get_current_user)):
    doc = await db.carousel_history.find_one(
        {"carousel_id": carousel_id, "org_id": current_user["org_id"]},
        {"_id": 0}
    )
    if not doc:
...
async def list_whatsapp_agents(current_user: dict = Depends(get_current_user)):
    agents = await db.whatsapp_agents.find({"org_id": current_user["org_id"]}, {"_id": 0}).sort("created_at", -1).to_list(100)
    return agents
...
async def update_whatsapp_agent(agent_id: str, body: WhatsAppAgentUpdate, current_user: dict = Depends(get_current_user)):
    ...
    if not update_data:
        raise HTTPException(status_code=400, detail="Nenhum campo para atualizar")
    update_data["updated_at"] = datetime.now(timezone.utc).isoformat()
    result = await db.whatsapp_agents.update_one({"agent_id": agent_id, "org_id": current_user["org_id"]}, {"$set": update_data})
    if result.matched_count == 0:
        raise HTTPException(status_code=404, detail="Agente não encontrado")
    return await db.whatsapp_agents.find_one({"agent_id": agent_id, "org_id": current_user["org_id"]}, {"_id": 0})
...
async def toggle_whatsapp_agent(agent_id: str, current_user: dict = Depends(get_current_user)):
    agent = await db.whatsapp_agents.find_one({"agent_id": agent_id, "org_id": current_user["org_id"]}, {"_id": 0})
    if not agent:
        raise HTTPException(status_code=404, detail="Agente não encontrado")
...
async def receive_whatsapp_lead(body: WhatsAppLeadPayload, token: str = ""):
    """
    Public endpoint (no auth) — N8N calls this when a WhatsApp message arrives.
    Requires ?token=<webhook_token> matching a client in the database.
    Creates a Lead automatically with source='whatsapp'.
    """
    client_record = await db.clients.find_one({"webhook_token": token}, {"_id": 0}) if token else None
...
async def receive_instagram_lead(body: InstagramLeadPayload, token: str = ""):
    """
    ...
    instagram_stage = await db.pipeline_stages.find_one(
        {"pipeline_type": "instagram", "org_id": org_id}, sort=[("order", 1)]
    )
    if instagram_stage:
...
async def receive_generic_lead(body: GenericLeadPayload, token: str = ""):
    """
...
async def instagram_webhook_receive(request: Request):
    ...
    # Verify HMAC-SHA1 signature from Meta
    meta_app_secret = os.environ.get("META_APP_SECRET", "")
    if meta_app_secret:
        body_bytes = await request.body()
        expected_sig = "sha1=" + hmac.new(
            meta_app_secret.encode(), body_bytes, hashlib.sha1
        ).hexdigest()
        ...
        try:
            payload = json_module.loads(body_bytes)
        except Exception:
            return {"ok": False}
    else:
        try:
...
async def list_instagram_conversations(current_user: dict = Depends(get_current_user)):
    convs = await db.instagram_conversations.find({"org_id": current_user["org_id"]}, {"_id": 0}).sort("updated_at", -1).to_list(200)
    result = []
    for c in convs:
        msgs = c.get("messages", [])
...
async def get_instagram_conversation(conversation_id: str, current_user: dict = Depends(get_current_user)):
    conv = await db.instagram_conversations.find_one({"conversation_id": conversation_id, "org_id": current_user["org_id"]}, {"_id": 0})
    if not conv:
